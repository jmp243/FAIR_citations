...
# ── 1. Build per-journal ScimagoJR lookup ────────────────────────────────────
# One row per unique journal; keep the SJR metadata we want to surface.
# alex_doi_new_journal already exists (ISSN join in topic_words_clean.R).
...
scimago_lookup <- alex_doi_new_journal %>%
  ...
  # If multiple ISSN rows matched the same journal, keep the best-ranked row
  group_by(journal_name) %>%
  arrange(sjr_rank) %>%
  slice(1) %>%
  ungroup()

# ── 2. Year × journal paper counts (unique DOIs) ─────────────────────────────
journal_year_counts <- alex_doi_new_journal %>%
  ...
  # Attach ScimagoJR metadata
  left_join(scimago_lookup, by = "journal_name")

# ── 3. Overall journal totals (for picker labels & default sort) ──────────────
journal_totals <- journal_year_counts %>%
  group_by(
    journal_name, h_index, sjr_quartile, sjr_rank, cites_per_doc, categories, areas
  ) %>%
  summarise(total_papers = sum(n_papers), .groups = "drop") %>%
  arrange(desc(total_papers))

# ── 4. Quartile factor levels (for ordering / colour) ────────────────────────
quartile_levels <- c("Q1", "Q2", "Q3", "Q4", NA_character_)

journal_totals <- journal_totals %>%
  mutate(
    sjr_quartile = factor(sjr_quartile, levels = quartile_levels),
    h_label      = if_else(!is.na(h_index),
                           paste0("h=", h_index),
                           "h=N/A"),
    quartile_label = if_else(!is.na(sjr_quartile),
                             as.character(sjr_quartile),
                             "Unranked")
  )

# ── 5. Selectize choices: "Journal (N papers | Q1 | h=42)" ───────────────────
jt_choices <- setNames(
...
# Quartile choices for the filter widget
jt_quartile_choices <- c("All", "Q1", "Q2", "Q3", "Q4", "Unranked")
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectizeInput(
        ...
        multiple = TRUE,
        options  = list(
          placeholder = "Search and add journals…",
          plugins     = list("remove_button"),
          maxItems    = 20
        )
      ),
      helpText("Labels show total FAIR papers | SJR quartile | h-index where available."),
      
      tags$hr(),
      
      # ── Quick-filter by SJR quartile ───────────────────────────────────────
      selectInput(
        "jt_quartile_filter",
        "Filter picker to quartile:",
        choices  = jt_quartile_choices,
        selected = "All"
      ),
      helpText("Narrows the journal list above; does not affect already-selected journals."),
    ...
    mainPanel(
      ...
      tabsetPanel(
        ...
        # ── Journal metadata card table ───────────────────────────────────────
        tabPanel(
          "Journal Profiles",
          br(),
          helpText("ScimagoJR 2025 metadata for selected journals. Rank and h-index are global across all fields."),
          DTOutput("jt_profile_table")
        ),
        # ── H-index scatter ───────────────────────────────────────────────────
        tabPanel(
          "Impact vs. FAIR Papers",
          br(),
          helpText("Bubble size = total FAIR-citing papers. X-axis = h-index. Colour = SJR quartile."),
...
# =============================================================================
# [SERVER]  Tab 4: Journal Trends
# =============================================================================

# ── Reactive: update picker choices when quartile filter changes ──────────────
observe({
  qf <- input$jt_quartile_filter
  
  filtered_journals <- if (qf == "All") {
    journal_totals
  } else if (qf == "Unranked") {
    journal_totals %>% filter(is.na(sjr_quartile) | quartile_label == "Unranked")
  } else {
    journal_totals %>% filter(quartile_label == qf)
  }
  
  new_choices <- setNames(
    filtered_journals$journal_name,
    paste0(
      filtered_journals$journal_name,
      " (", filtered_journals$total_papers, " papers",
      if_else(!is.na(filtered_journals$sjr_quartile),
              paste0(" | ", filtered_journals$sjr_quartile), ""),
      if_else(!is.na(filtered_journals$h_index),
              paste0(" | h=", filtered_journals$h_index), ""),
      ")"
    )
  )
  
  # Preserve any already-selected journals even if outside the filter
  current_sel <- isolate(input$jt_journals)
  valid_sel   <- intersect(current_sel, filtered_journals$journal_name)
...
# ── Reactive: compute display values ─────────────────────────────────────────
jt_display <- reactive({
  dat <- jt_filtered()
  
  if (input$jt_y_mode == "percent") {
    year_totals_all <- journal_year_counts %>%
      filter(
        publication_year >= input$jt_year_range[1],
        publication_year <= input$jt_year_range[2]
      ) %>%
    ...
    dat <- dat %>%
      left_join(year_totals_all, by = "publication_year") %>%
      mutate(
        value   = if_else(grand_total > 0, 100 * n_papers / grand_total, 0),
        y_label = "% of all FAIR-citing papers that year"
      )
  } else {
    dat <- dat %>% mutate(value = n_papers,
                          y_label = "FAIR-citing papers (unique DOIs)")
  }
  dat
})
...
jt_pal <- reactive({
  ...
  if (color_by == "quartile") {
    # Fixed quartile colours
    c(Q1 = "#1a9641", Q2 = "#a6d96a", Q3 = "#fdae61", Q4 = "#d7191c",
      Unranked = "#bdbdbd")
  } else {
    journals <- sort(unique(dat$journal_name))
    n        <- length(journals)
    cols     <- if (n <= 10) scales::hue_pal()(n) else viridis::viridis(n, option = "turbo")
...
output$jt_trend_plot <- renderPlotly({
  ...
  # Map each journal to its colour key
  get_col <- function(row_journal, row_quartile) {
    if (color_by == "quartile") {
      pal[coalesce(row_quartile, "Unranked")]
    } else {
      pal[row_journal]
    }
  }
...
# ── Output: journal profile card table ───────────────────────────────────────
output$jt_profile_table <- renderDT({
  req(input$jt_journals)
  
  profile_dat <- journal_totals %>%
  ...
  datatable(
    ...
    options  = list(
      ...
      scrollX    = TRUE,
      columnDefs = list(
        list(width = "300px", targets = c(6, 7))   # wider for Categories/Areas
      )
    )
  ) %>%
    formatStyle(
...
# ── Output: h-index vs. FAIR papers scatter ───────────────────────────────────
output$jt_scatter_plot <- renderPlotly({
  ...
  quartile_pal <- c(
    Q1 = "#1a9641", Q2 = "#a6d96a", Q3 = "#fdae61",
    Q4 = "#d7191c", Unranked = "#bdbdbd"
  )
